...
[![Empirical Validation](https://img.shields.io/badge/Active%20Phase%20Recall-90.1%25-emerald)](#empirical-ground-truth-benchmarks)
...
- **Secondary Diagnostic Context: Full-Trajectory Audit (N=22,000):**
  * Pre-Fault Normal Specificity: **`92.2%`** (`0.9215`) — *Correctly recognizes healthy state prior to fault onset*
  * Point-wise Overall Accuracy: **`56.1%`** (`0.5614`) — *Reflects healthy lead-in steps (~350s) labeled under run-level fault IDs*
...
- **Anomaly Detection (Isolation Forest, Normal-Trained N=7,427):**
  ...
  * False Alarm Rate ($\alpha$): **`5.7%`** (`0.0570`, suppressed by temporal persistence)
